...
router.post(
  "/login",
  passport.authenticate("local", {  // 使用local驗證的middleware
    failureRedirect: "/auth/login",   // 登入失敗時，會重新導向的地方
    failureFlash: "登入失敗，帳號或密碼不正確",   // 登入失敗會顯示的提示 (會被套用在index.js裡的res.locals.error)
  }),
  (req, res) => {
...
router.get("/google/redirect", passport.authenticate("google"), (req, res) => {
  // 因為這個route要通過google驗證才能使用，所以要authenticate
